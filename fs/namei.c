...
static inline int __follow_down(struct vfsmount **mnt, struct dentry **dentry)
{
	...
	while (p != &(*dentry)->d_vfsmnt) {
        ...
        /*ZcNote::cnmb明白了，这个parent不一定是一级目录*/
...
/*ZcNote::".."的情况,比如首次到这里，如果nd是绝对路径是/data/apps/dataserver
                          而nd是相对路径，从dataserver开始出发，然后name是../mnt/dir1
                          也就是dataserver/../mnt/dir1 而这个/data/apps/dataserver 恰好是一个挂载点 
                          所以apps是另一个文件系统，这就是../的麻烦之处*/

/*ZcNote::这时nd还是/data/apps/dataserver */
...
int path_walk(const char * name, struct nameidata *nd)
{
    ...
    /*ZcNote::直到这时nd还是第一级那个目录，无论是'/'还是那个相对路径*/
	for(;;) {
		unsigned long hash;
		struct qstr this;
		unsigned int c;
		...
		dentry = ERR_PTR(err);
 		if (err)
			break;
        /*ZcNote::这个this是一个临时变量，是name加len加hash，用于后面的各种lookup*/
		this.name = name;
		...
		while (*++name == '/');
		if (!*name)
            /*ZcNote::这里到了最后一级，所以到了last_with_slashes*/
		...
		if (this.name[0] == '.') switch (this.len) {
			default:
				break;
			case 2:	
                /*ZcNote::就是..的情况,比如首次到这里，如果nd是绝对路径是/data/apps/dataserver/dir
                          而nd是相对路径，从dataserver开始出发，然后name是../mnt/dir1
                          也就是dataserver/../mnt/dir1 而这个/data/apps 恰好是一个挂载点       */
				if (this.name[1] != '.')
					break;
				follow_dotdot(nd);
				inode = nd->dentry->d_inode;
