...
dirty_page_rescan:
    ...
    /*ZcNote::首先这个地方是对inactive dirty的page做了循环，而且是对所有的page，注意，是所有的
              page，因为这个maxscan是不变了，就是所有的page个数*/
	while ((page_lru = inactive_dirty_list.prev) != &inactive_dirty_list &&
		...
		/* Page is or was in use?  Move it to the active list. */
        /*ZcNote::PageTestandClearReferenced返回原始值!=0，如果原始值是1，1!=0 这里就返回1，也就是说
                  这里清理了reference，不太清楚为啥要clear一下*/
		if (PageTestandClearReferenced(page) || page->age > 0 ||
				(!page->buffers && page_count(page) > 1) ||
				page_ramdisk(page)) {
